...
local function refresh(network, chests, items, transferInventory)
	...
	items = sitems.loadItems(network, chests)
	chests, storageMax, storageUsed = sitems.loadChests(network, transferInventory)
	itemdisplay = sitems.loadIDisplay(items)
...
local function sort(network, chests, items, transferInventory)
	...
	sitems.combine(network, chests, transferInventory, items)
	chests, storageMax, storageUsed = sitems.loadChests(network, transferInventory)
	items = sitems.loadItems(network, chests)
	itemdisplay = sitems.loadIDisplay(items)
	term.setCursorBlink(true)
...
local function push(network, chests, items, transferInventory)
	...
	local maxMax
	if transferInventory==network.getNameLocal() then
		maxMax = 16
	else
		maxMax = network.callRemote(transferInventory,"size")
	end
	for i=1,maxMax do
		chests, _ = sitems.send(network, chests, transferInventory, i)
	end
	chests, storageMax, storageUsed = sitems.loadChests(network, transferInventory)
	items = sitems.loadItems(network, chests)
	itemdisplay = sitems.loadIDisplay(items)
		...
		if _key==keys.enter then
			if not _G.ME["prOpen"] then
				...
				if tonumber(_G.ME["amountString"]) then
					...
					items, chests, _ = sitems.get(network, chests, transferInventory, items, _G.ME["prSelected"], tonumber(_G.ME["amountString"]))
					_G.ME["prOpen"] = false
					chests, storageMax, storageUsed = sitems.loadChests(network, transferInventory)
					itemdisplay = sitems.loadIDisplay(items)
					term.setCursorBlink(true)
